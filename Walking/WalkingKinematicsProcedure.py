...
#             #Index
#             if argrelextrema(TabVGrf, np.greater_equal)[0].shape[0] > 2:
#                 print("PLUS DE DEUX")
#                 LocalMaxDataFrame = pd.DataFrame()
#                 LocalMaxDataFrame["ListIndexMax"] = list(argrelextrema(TabVGrf, np.greater_equal)[0])
...
#                 FirtPeakIndex = LocalMaxDataFrame["ListIndexMax"][0]
#                 SecondPeakIndex = -1
#                 for i in np.arange(len(LocalMaxDataFrame)):
#                     if SecondPeakIndex == -1 :
#                         if LocalMaxDataFrame["ListIndexMax"][i] > FirtPeakIndex + 1/3 * len(TabVGrf):
#                             SecondPeakIndex = LocalMaxDataFrame["ListIndexMax"][i]
#             else :
#                 FirtPeakIndex = argrelextrema(TabVGrf, np.greater_equal)[0][0]
#                 SecondPeakIndex = argrelextrema(TabVGrf, np.greater_equal)[0][1]

#             MidstanceValleyIndex = -1
#             for index in argrelextrema(TabVGrf[FirtPeakIndex : SecondPeakIndex], np.less_equal)[0]:
#                 if MidstanceValleyIndex == -1:
#                     if  FirtPeakIndex < index and index < SecondPeakIndex:
#                         MidstanceValleyIndex = index + FirtPeakIndex
